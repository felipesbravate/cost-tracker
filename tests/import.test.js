...
const fake = { years: [{ year: '2025', currency: 'EUR',
  incomes: [{ item: 'Salary', values: [1000, 1000, 0, ...Array(9).fill(0)], notes: [ 'Employer A\nBonus (5/1)', null, ...Array(10).fill(null)], realAmounts: [ [900, 100], null, ...Array(10).fill(null)] }],
  investments: [{ item: 'Savings', values: [50, ...Array(11).fill(0)] }],
  expenses: { Fixed: [{ item: 'Rent', category: 'Habitation', values: [400, 400, -20, ...Array(9).fill(0)], notes: [null, null, null, 'Ghost', ...Array(8).fill(null)] }], Variable: [], Extra: [], Additional: [] } }] };
...
test('converts non-zero cells into entries and years', () => {
  ...
  assert.equal(salary.note, 'Employer A\nBonus (5/1)'); assert.deepEqual(salary.realAmounts, [900, 100]);
  assert.equal(r.entries.filter((e) => e.item === 'Salary')[1].note, undefined);
